...
namespace ResultOf
{
    ...
    public static class Result
    {
        ...
        public static Result<T> Validate<T>(this T input, Func<T, bool> validate, string error)
        {
            return validate(input) ? Ok(input) : Fail<T>(error);
        }
        ...
        public static Result<TInput> OnSuccess<TInput>(
            this Result<TInput> input,
            Action<TInput> handleValue)
        {
            if (input.IsSuccess) handleValue(input.Value);
            return input;
        }

        public static Result<TInput> OnAnyResult<TInput>(
            this Result<TInput> input,
            Action action)
        {
            action();
